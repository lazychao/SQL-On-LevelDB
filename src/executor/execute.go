...
//关闭后，stopChannel才会关闭
func Execute(dataChannel <-chan types.DStatements, finishChannel chan<- error, operationChannel chan<- db.DbOperation) {
	var err Error.Error
	mapping.SetDbChannel(operationChannel)
	db.SetFinishChannel(finishChannel)

...
type Value interface {
	...
	Convert2BytesComparable() ([]byte, error)
...
func (i Int) Convert2Bytes() ([]byte, error) {
	...
	binary.Write(bytebuf, binary.BigEndian, i.Val)
	return bytebuf.Bytes(), nil
}
func (i Int) Convert2BytesComparable() ([]byte, error) {
	bytebuf := bytes.NewBuffer([]byte{})
	u := uint64(i.Val) ^ signMask //首位取反
	binary.Write(bytebuf, binary.BigEndian, u)
	return bytebuf.Bytes(), nil
}
...
func (i Float) Convert2BytesComparable() ([]byte, error) {
	bytebuf := bytes.NewBuffer([]byte{})
	u := math.Float64bits(i.Val)
	if i.Val >= 0 {
		//是正数，首位取反
		u |= signMask
	} else {
...
func (i Bytes) Convert2BytesComparable() ([]byte, error) {
	//TODO 如果有多列索引要扩展字节数组到相同的长度
	return i.Val, nil
}
...
func (i Bool) Convert2BytesComparable() ([]byte, error) {
	if i.Val {
		return []byte{1}, nil
	}
	return []byte{0}, nil
...
func (i Alien) Convert2BytesComparable() ([]byte, error) {
	bytebuf := bytes.NewBuffer([]byte{})
	binary.Write(bytebuf, binary.BigEndian, i.Val)
	return bytebuf.Bytes(), nil
}
...
func (i Null) Convert2BytesComparable() ([]byte, error) {
	return make([]byte, i.Length), nil
}
...
func Byte2ValueComparable(mybytes []byte, vt ValueType, length ...int) (Value, error) {
	switch vt {
	case BoolType:
		if len(mybytes) < 1 {
			return nil, errors.New("mybytes length is less than 1")
	...
	case IntType:
		...
		var retu uint64
		buf := bytes.NewBuffer(mybytes[0:8])
		binary.Read(buf, binary.BigEndian, &retu)
		ret := int64(retu ^ signMask)
		return Int{Val: ret}, nil
	case FloatType:
		if len(mybytes) < 8 {
			return nil, errors.New("mybytes length is less than 8")
		...
		return Float{Val: math.Float64frombits(retu)}, nil
	case BytesType:
		if len(length) < 1 || length[0] <= 0 {
			return nil, errors.New("please input a length for bytes")
		}
